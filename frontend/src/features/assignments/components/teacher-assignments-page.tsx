...
const columns: readonly DataTableColumn<AssignmentListItem>[] = [
  ...
  },
  {
    ...
    cell: (x) =>
      new Intl.DateTimeFormat("en-BD", {
        dateStyle: "medium",
        timeStyle: "short",
      }).format(new Date(x.deadlineUtc)),
  },
  { id: "marks", header: "Marks", cell: (x) => x.maximumMarks },
  {
    id: "submissions",
    header: "Submissions",
  ...
  },
  {
    ...
    header: "Status",
    cell: (x) => (
      <StatusBadge label={x.status} status={x.status.toLowerCase()} />
    ),
  },
  {
    id: "action",
    ...
    cell: (x) => (
      <Button asChild size="sm" variant="outline">
        <Link href={`/teacher/assignments/${x.id}`}>
...
export function TeacherAssignmentsPage() {
  ...
  function setFilter(name: string, value?: string) {
    ...
    router.replace(`/teacher/assignments?${next}`);
  }
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Teacher"
        title="Assignments"
        description="Create, publish, close, and monitor your assignments."
        actions={
          <Button asChild>

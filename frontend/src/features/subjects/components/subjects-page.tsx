"use client";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import { Eye } from "lucide-react";
import Link from "next/link";
import { useRouter, useSearchParams } from "next/navigation";
import { useState } from "react";
import {
  DataTable,
  type DataTableColumn,
} from "@/shared/components/data-table/data-table";
import { Pagination } from "@/shared/components/data-table/pagination";
import { StatusBadge } from "@/shared/components/data-table/status-badge";
import { ErrorState } from "@/shared/components/feedback/error-state";
import { LoadingState } from "@/shared/components/feedback/loading-state";
import { FilterBar } from "@/shared/components/filters/filter-bar";
import { SearchInput } from "@/shared/components/filters/search-input";
import { PageHeader } from "@/shared/components/layout/page-header";
import { Button } from "@/shared/components/ui/button";
import { Label } from "@/shared/components/ui/label";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/shared/components/ui/select";
import { getSubjects } from "../subjects.api";
import type { Subject, SubjectFilters } from "../subjects.types";
import { CreateSubjectDialog } from "./create-subject-dialog";

const columns: readonly DataTableColumn<Subject>[] = [
  {
    id: "subject",
    header: "Subject",
    cell: (subject) => (
      <div>
        <p className="font-medium">{subject.code}</p>
        <p className="text-muted-foreground text-xs">{subject.name}</p>
      </div>
    ),
  },
  {
    id: "description",
    header: "Description",
    cell: (subject) => (
      <span className="line-clamp-2 max-w-lg">
        {subject.description ?? "—"}
      </span>
    ),
  },
  {
    id: "status",
    header: "Status",
    cell: (subject) => (
      <StatusBadge
        label={subject.isActive ? "Active" : "Inactive"}
        status={subject.isActive ? "active" : "inactive"}
      />
    ),
  },
  {
    id: "created",
    header: "Created",
    cell: (subject) =>
      new Intl.DateTimeFormat("en-BD", { dateStyle: "medium" }).format(
        new Date(subject.createdAtUtc),
      ),
  },
  {
    id: "action",
    header: "",
    className: "text-right",
    cell: (subject) => (
      <Button asChild size="sm" variant="outline">
        <Link href={`/admin/subjects/${subject.id}`}>
          <Eye aria-hidden="true" />
          View
        </Link>
      </Button>
    ),
  },
];

export function SubjectsPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const queryClient = useQueryClient();
  const filters: SubjectFilters = {
    pageNumber: Math.max(Number(searchParams.get("page")) || 1, 1),
    pageSize: 20,
    search: searchParams.get("search") || undefined,
    isActive: searchParams.has("active")
      ? searchParams.get("active") === "true"
      : undefined,
  };
  const [search, setSearch] = useState(filters.search ?? "");
  const query = useQuery({
    queryKey: ["subjects", "list", filters],
    queryFn: () => getSubjects(filters),
  });
  function setFilter(name: string, value?: string) {
    const next = new URLSearchParams(searchParams);
    if (value) next.set(name, value);
    else next.delete(name);
    if (name !== "page") next.delete("page");
    router.replace(`/admin/subjects?${next}`);
  }
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Administration"
        title="Subjects"
        description="Manage subjects used across course teaching assignments."
        actions={
          <CreateSubjectDialog
            onCreated={() =>
              void queryClient.invalidateQueries({ queryKey: ["subjects"] })
            }
          />
        }
      />
      <FilterBar>
        <SearchInput
          value={search}
          placeholder="Search code or name"
          onValueChange={(value) => {
            setSearch(value);
            setFilter("search", value);
          }}
        />
        <div className="space-y-1.5">
          <Label>Status</Label>
          <Select
            value={
              filters.isActive === undefined ? "all" : String(filters.isActive)
            }
            onValueChange={(value) =>
              setFilter("active", value === "all" ? undefined : value)
            }
          >
            <SelectTrigger className="w-40" aria-label="Filter by status">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All statuses</SelectItem>
              <SelectItem value="true">Active</SelectItem>
              <SelectItem value="false">Inactive</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </FilterBar>
      {query.isPending ? <LoadingState /> : null}
      {query.isError ? (
        <ErrorState
          description={query.error.message}
          onRetry={() => void query.refetch()}
        />
      ) : null}
      {query.data ? (
        <>
          <DataTable
            columns={columns}
            items={query.data.items}
            getRowKey={(subject) => subject.id}
            emptyTitle="No subjects found"
          />
          <Pagination
            pageNumber={query.data.pageNumber}
            totalPages={query.data.totalPages}
            onPageChange={(page) => setFilter("page", String(page))}
          />
        </>
      ) : null}
    </div>
  );
}
